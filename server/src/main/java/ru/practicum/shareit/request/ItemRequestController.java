...
public class ItemRequestController {
    ...
    @GetMapping("/all")
    public ResponseEntity<List<ItemRequestDto>> getAllRequests(@RequestHeader(USER_HEADER) Long userId) {
        List<ItemRequestDto> list = requestService.getAllRequests(userId);
        return ResponseEntity.ok(list);
    }

    @GetMapping("/{requestId}")
    public ResponseEntity<ItemRequestDto> getRequestById(@RequestHeader(USER_HEADER) Long userId,
